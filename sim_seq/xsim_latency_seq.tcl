...
if {![file exists [file join $repo_root "rtl_seq" "mimo_detector_top_seq.sv"]]} {
    set cwd_root [file normalize [pwd]]

    if {[file exists [file join $cwd_root "rtl_seq" "mimo_detector_top_seq.sv"]]} {
...
set work_dir [file normalize [file join $repo_root "build" "xsim_latency_seq"]]
set empty_include_dir [file join $work_dir "empty_include"]
set results_dir [file normalize [file join $repo_root "results" "latency_seq"]]
file mkdir $work_dir
...
puts "============================================================"
puts "XSIM sequential detector latency simulation"
...
set sv_files [list \
    [file join $repo_root "tb" "phase6_vectors_pkg.sv"] \
    ...
    [file join $repo_root "tb_seq" "tb_mimo_detector_latency_seq.sv"] \
]
...
run_cmd $xelab -nolog -debug typical --timescale 1ns/1ps --override_timeunit --override_timeprecision ${work_lib}.tb_mimo_detector_latency_seq -s tb_mimo_detector_latency_seq_sim
set sim_output [run_cmd $xsim -nolog tb_mimo_detector_latency_seq_sim -runall]

if {[string first "tb_mimo_detector_latency_seq PASSED" $sim_output] < 0} {
    puts ""
    puts "FAIL: tb_mimo_detector_latency_seq did not report PASSED"
    exit 1
}

if {[string first "Error:" $sim_output] >= 0 || [string first "FAILED" $sim_output] >= 0} {
    puts ""
    puts "FAIL: tb_mimo_detector_latency_seq reported simulation errors"
    exit 1
}

puts ""
puts "PASS: tb_mimo_detector_latency_seq completed"
puts "Report: [file join $results_dir latency_seq_report.txt]"
puts "CSV:    [file join $results_dir latency_seq_summary.csv]"
exit 0
