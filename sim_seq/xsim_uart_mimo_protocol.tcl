...
set work_dir [file normalize [file join $repo_root "build" "xsim_uart_mimo_protocol"]]
...
puts "============================================================"
puts "XSIM UART MIMO protocol simulation"
...
set sv_files [list \
    ...
    [file join $repo_root "tb_seq" "tb_uart_mimo_protocol.sv"] \
]
...
run_cmd $xelab -nolog -debug typical --timescale 1ns/1ps --override_timeunit --override_timeprecision ${work_lib}.tb_uart_mimo_protocol -s tb_uart_mimo_protocol_sim
set sim_output [run_cmd $xsim -nolog tb_uart_mimo_protocol_sim -runall]

if {[string first "tb_uart_mimo_protocol PASSED" $sim_output] < 0} {
    puts ""
    puts "FAIL: tb_uart_mimo_protocol did not report PASSED"
    exit 1
}

if {[string first "Error:" $sim_output] >= 0 || [string first "FAILED" $sim_output] >= 0} {
    puts ""
    puts "FAIL: tb_uart_mimo_protocol reported simulation errors"
    exit 1
}

puts ""
puts "PASS: tb_uart_mimo_protocol completed"
exit 0
